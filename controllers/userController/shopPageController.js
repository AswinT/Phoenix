  ...
  try {
    ...
    if (categoryId) {
      ...
      if (Array.isArray(categoryId)) {
        // Check if it's ObjectId format or category name
        const categoryQuery = categoryId.map(id => {
          if (id.match(/^[0-9a-fA-F]{24}$/)) {
            return id; // Valid ObjectId
          } else {
            // Find category by name (this will be handled later)
            return id;
          }
        });
        query.category = { $in: categoryQuery };
      } else {
        // Single category - check if ObjectId or name
        if (categoryId.match(/^[0-9a-fA-F]{24}$/)) {
          query.category = categoryId; // Valid ObjectId
        } else {
          // Find category by name - we'll need to look it up
          try {
            ...
            if (category) {
              ...
              // Invalid category name - return empty results
              query.category = null;
            }
          } catch (error) {
            console.log('Error finding category by name:', error);
            query.category = null;
          }
        }
      }
