...
/**
 * Cancel an order from admin side
...
const searchOrders = async (req, res) => {
  try {
    ...
    // Text search in order ID or user details
    if (query) {
      searchCriteria.$or = [
...
const filterOrders = async (req, res) => {
  try {
    const { status, dateFrom, dateTo, minAmount, maxAmount, paymentStatus } = req.query;
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 10;
    const skip = (page - 1) * limit;
    ...
    if (paymentStatus && paymentStatus !== 'all') {
      filterCriteria.paymentStatus = paymentStatus;
    }
    ...
    if (dateFrom || dateTo) {
      ...
      if (dateFrom) {
        filterCriteria.createdAt.$gte = new Date(dateFrom);
      }
      if (dateTo) {
        filterCriteria.createdAt.$lte = new Date(dateTo);
      }
    }
    ...
    if (minAmount || maxAmount) {
      ...
      if (minAmount) {
        filterCriteria.totalAmount.$gte = parseFloat(minAmount);
      }
      if (maxAmount) {
        filterCriteria.totalAmount.$lte = parseFloat(maxAmount);
      }
    }
